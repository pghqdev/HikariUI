...
| `command` / `commandfor` invokers | 135 | 26.2 | 144 | Opening `<dialog>` from a button | `hikarion.js` polyfills the invoker click. With neither native support nor the script, the dialog cannot be opened — the one place where no-JS and an old browser compound. |
