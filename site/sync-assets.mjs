...
// The framework files have stable names, so a CDN happily serves a stale copy
// for hours after a deploy — fresh HTML against yesterday's CSS, which reads as
// "the site is broken". Fingerprint the link instead of shortening the cache.
const version = createHash("sha256")
  .update(readFileSync(from("dist/hikarion.min.css")))
  .update(readFileSync(from("dist/hikarion.js")))
  .digest("hex")
  .slice(0, 8);
writeFileSync(
  fileURLToPath(new URL("src/assets.mjs", import.meta.url)),
  `// Generated by sync-assets.mjs. Cache-busting stamp for the framework files.\nexport const ASSET_V = ${JSON.stringify(version)};\n`,
);
...
for (const f of ["kitchen-sink.html", "dashboard.html", "settings.html", "auth.html"]) {
  ...
  writeFileSync(p, readFileSync(p, "utf8").replace(
    /(href|src)="(?:\.\.?\/)*dist\/([\w.-]+)"/g,
    (m, attr, file) => `${attr}="../${file}?v=${version}"`,
  ));
}
